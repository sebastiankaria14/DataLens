...
def _resolve_ml_ready_files(dataset: Dataset) -> Dict[str, Optional[str]]:
    """Resolve expected train/test ML-ready parquet paths for a dataset."""
    profile = dataset.profile_data or {}
    ml_stats = profile.get("ml_preparation") or {}

    train_path = ml_stats.get("train_path")
    test_path = ml_stats.get("test_path")

    if not train_path or not test_path:
        stem = Path(dataset.file_path).stem.replace("cleaned_", "")
        ml_dir = Path(settings.PROCESSED_DIR) / "ml_ready"
        if not train_path:
            train_path = str(ml_dir / f"train_{stem}.parquet")
        if not test_path:
            test_path = str(ml_dir / f"test_{stem}.parquet")
...
@router.get("/{dataset_id}/ml-ready-files")
async def get_ml_ready_files(
    dataset_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    """Return availability metadata for ML-ready train/test files."""
    dataset = db.query(Dataset).filter(
        Dataset.id == dataset_id,
        Dataset.owner_id == current_user.id,
    ).first()
    ...
    resolved = _resolve_ml_ready_files(dataset)
    train_file = Path(resolved["train_path"]) if resolved.get("train_path") else None
    test_file = Path(resolved["test_path"]) if resolved.get("test_path") else None

    return {
        "train_available": bool(train_file and train_file.exists()),
        "test_available": bool(test_file and test_file.exists()),
        "train_filename": train_file.name if train_file else None,
        "test_filename": test_file.name if test_file else None,
    }


@router.get("/{dataset_id}/download-ml-ready")
async def download_ml_ready_file(
    dataset_id: int,
    split: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    """Download train or test parquet generated by ML preparation."""
    ...
    resolved = _resolve_ml_ready_files(dataset)
    file_key = "train_path" if split_lower == "train" else "test_path"
    file_path = resolved.get(file_key)
    if not file_path:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="ML-ready files are not available yet")

    path_obj = Path(file_path)
    if not path_obj.exists():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="ML-ready file not found on disk")
