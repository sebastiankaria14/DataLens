...
const Login: React.FC = () => {
  ...
  return (
    <div className="page-shell lg:grid lg:grid-cols-[0.95fr_1.05fr]">
      <div className="hidden lg:flex bg-slate-900 flex-col justify-between p-12 relative overflow-hidden">
        <div className="absolute inset-0 opacity-[0.05]" style={{ backgroundImage: 'linear-gradient(#fff 1px,transparent 1px),linear-gradient(90deg,#fff 1px,transparent 1px)', backgroundSize: '34px 34px' }} />
        <div className="relative z-10 flex items-center gap-3">
          <div className="w-9 h-9 bg-sky-700 rounded-xl flex items-center justify-center">
        ...
        <div className="relative z-10 space-y-8">
          <div>
            <h2 className="text-3xl font-bold text-white leading-tight">Professional dataset operations,<br />without heavy tooling overhead.</h2>
            <p className="mt-4 text-slate-400 text-sm leading-relaxed max-w-sm">Centralize ingestion, profiling, cleaning, and export workflows in a single interface tailored for ML teams.</p>
          </div>
          <div className="space-y-4">
            {([['Engineered profiling','DuckDB queries directly from persisted files'],['Reliable cleaning','Deterministic preprocessing with Polars'],['Deployment-ready output','Export in formats your pipelines already use']] as const).map(([l,s])=>(
              <div key={l} className="flex items-start gap-3">
                <div className="w-5 h-5 rounded-full bg-sky-500/20 flex items-center justify-center flex-shrink-0 mt-0.5"><div className="w-2 h-2 rounded-full bg-sky-300"/></div>
      ...
      <div className="flex items-center justify-center px-6 py-12">
        <motion.div initial={{ opacity:0,y:16 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.4,ease:'easeOut' }} className="w-full max-w-md surface-pane p-7 sm:p-8">
          <div className="lg:hidden flex items-center gap-2 mb-8">
            <div className="w-8 h-8 bg-sky-800 rounded-lg flex items-center justify-center"><svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4"/></svg></div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" autoComplete="on">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <Link to="/forgot-password" className="text-xs text-sky-700 hover:text-sky-800 font-medium">Forgot password?</Link>
              </div>
          ...
          <p className="mt-6 text-center text-sm text-slate-500">No account?{' '}<Link to="/signup" className="text-sky-700 hover:text-sky-800 font-medium">Create one</Link></p>
        </motion.div>
      </div>
    </div>
  );
